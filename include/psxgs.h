...
/*
An open-source re-implementation of Sony's libgs
graphics library.

This is the top-level file you actually want to include.
The other ones are internal implementation details, don't use them!
*/
...
typedef struct _GsCOORDINATE {
	...
	MATRIX  coord, workm;
	GsCOORD2PARAM *param;
	struct _GsCOORDINATE *super, *sub;
} GsCOORDINATE;
...
typedef struct {
	unsigned long length, offset, point;
	GsOT_Tag *org, *tag;
} GsOT;
...
typedef struct {
	GsCOORDINATE *coord;
	unsigned long *tmd, *packet, id, attribute;
} GsDOBJ5;

typedef struct {
	unsigned long attribute;
	short x, y, cx, cy, mx, my, scalex, scaley;
	unsigned short w, h, tpage;
	unsigned char u, v, r, g, b;
	long rotate;
} GsSPRT;
...
typedef struct {
	...
	unsigned short ncellw, ncellh, *index;
	GsCELL *base;
} GsMAP;

typedef struct {
	unsigned long attribute;
	short x, y, w, h, scrollx, scrolly, mx, my, scalex, scaley;
	unsigned char r, g, b;
	GsMAP  *map;
	long    rotate;
} GsBG;
...
typedef struct {
	unsigned long pmode, *clut, *pixel;
	short  px, py, cx, cy;
	unsigned short 	pw, ph, cw, ch; 
} GsTIM;
...
#define GsIncFrame()  (PSDCNT++, PSDCNT= PSDCNT?PSDCNT:1, (PSDIDX= (PSDIDX==0?1:0)))
...
/*** TMD structure ****/
typedef struct {
	u_char  out, in, dummy, cd, r0, g0, b0, code;
	u_short n0, v0, v1, v2;
} GsTMD_PF3;
...
void GsLookAt(VECTOR *eye, VECTOR *target, SVECTOR *up, MATRIX *mtx);
...
void GsLoadTIM(void);
	...
	void    GsSortSPRT(GsSPRT * sp, GsOT * ot, unsigned short pri);
	void    GsSortSPRTB(GsSPRT * sp, GsOT * ot, unsigned short pri,
				unsigned short flip);
	void    GsSortFastSPRT(GsSPRT * sp, GsOT * ot, unsigned short pri);
	void    GsSortFastSPRTB(GsSPRT * sp, GsOT * ot, unsigned short pri,
				unsigned short flip);
	void    GsSortFlipSPRT(GsSPRT * sp, GsOT * ot, unsigned short pri);
	void    GsSortBB(GsBG * bg, GsOT * ot, unsigned short pri);
	...
	void    GsGetLw(GsCOORDINATE * m, MATRIX * out);
	void    GsGetLs(GsCOORDINATE * m, MATRIX * out);
...
extern _GsPOSITION GsPOS;	/* 2d offset */
